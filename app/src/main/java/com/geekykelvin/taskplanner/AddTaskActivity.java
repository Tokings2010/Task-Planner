...
import com.geekykelvin.taskplanner.reminder.TaskScheduler;
...
public class AddTaskActivity extends AppCompatActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Setup TextViews using Task values
    ...
    public void setTime(View v){
        if(mCurrentTaskUri == null){
            Toast.makeText(this, "click again on the Task list to set time alarm", Toast.LENGTH_LONG).show();
            return;
        }
    ...
    public void setDate(View v){
        if(mCurrentTaskUri == null){
            Toast.makeText(this, "click again on the Task list to set date alarm", Toast.LENGTH_LONG).show();
            return;
        }
        Calendar now = Calendar.getInstance();
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (mCurrentTaskUri == null) {
            MenuItem menuItem = menu.findItem(R.id.discard_task);
        ...
        switch (item.getItemId()) {
            ...
            case R.id.save_task:
                ...
                if (mTitleText.getText().toString().length() == 0){
                    mTitleText.setError("Task Title cannot be blank!");
                }
    ...
    private void deleteTask() {
        ...
        if (mCurrentTaskUri != null) {
            ...
            new TaskScheduler().cancelAlarm(getApplicationContext(), mCurrentTaskUri);
...
*/
        ...
        values.put(TaskContract.AlarmTaskEntry.KEY_TITLE, mTitle);
        values.put(TaskContract.AlarmTaskEntry.KEY_DATE, mDate);
        values.put(TaskContract.AlarmTaskEntry.KEY_TIME, mTime);
        values.put(TaskContract.AlarmTaskEntry.KEY_REPEAT, mRepeat);
        values.put(TaskContract.AlarmTaskEntry.KEY_REPEAT_NO, mRepeatNo);
        values.put(TaskContract.AlarmTaskEntry.KEY_REPEAT_TYPE, mRepeatType);
        values.put(TaskContract.AlarmTaskEntry.KEY_ACTIVE, mActive);
        ...
        if (mActive.equals("true")) {
            if (mRepeat.equals("true")) {
                new TaskScheduler().setRepeatAlarm(getApplicationContext(), selectedTimestamp, mCurrentTaskUri, mRepeatTime);
            } else if (mRepeat.equals("false")) {
                new TaskScheduler().setAlarm(getApplicationContext(), selectedTimestamp, mCurrentTaskUri);
            }
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        ...
        if (cursor.moveToFirst()) {
            int titleColumnIndex = cursor.getColumnIndex(TaskContract.AlarmTaskEntry.KEY_TITLE);
            int dateColumnIndex = cursor.getColumnIndex(TaskContract.AlarmTaskEntry.KEY_DATE);
            int timeColumnIndex = cursor.getColumnIndex(TaskContract.AlarmTaskEntry.KEY_TIME);
            int repeatColumnIndex = cursor.getColumnIndex(TaskContract.AlarmTaskEntry.KEY_REPEAT);
            int repeatNoColumnIndex = cursor.getColumnIndex(TaskContract.AlarmTaskEntry.KEY_REPEAT_NO);
            int repeatTypeColumnIndex = cursor.getColumnIndex(TaskContract.AlarmTaskEntry.KEY_REPEAT_TYPE);
            int activeColumnIndex = cursor.getColumnIndex(TaskContract.AlarmTaskEntry.KEY_ACTIVE);
