...
import com.geekykelvin.taskplanner.AddTaskActivity;
import com.geekykelvin.taskplanner.R;
...
public class TaskAlarmService extends IntentService {
    ...
    public static PendingIntent getReminderPendingIntent(Context context, Uri uri) {
        Intent action = new Intent(context, TaskAlarmService.class);
        action.setData(uri);
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        try {
            if (cursor != null && cursor.moveToFirst()) {
                description = TaskContract.getColumnString(cursor, TaskContract.AlarmTaskEntry.KEY_TITLE);
            }
        } finally {
            if (cursor != null) {
                cursor.close();
